...
func TestNetpollConnResumeOnNextReadRequiresDrainBeforeReentry(t *testing.T) {
	writer := newCountingNetpollWriterForTest()
	reader := &scriptedNetpollReaderForTest{data: []byte("partial")}
	raw := &countingNetpollConnectionForTest{
		writer: writer,
		reader: reader,
		local:  testNetpollAddr("127.0.0.1:8080"),
	...
	if got := calls.Load(); got != 1 {
		t.Fatalf("OnData calls after first serve = %d, want 1", got)
	}
	...
	if got := calls.Load(); got != 1 {
		t.Fatalf("OnData calls with stale buffered data = %d, want 1", got)
	}
	...
	if got := calls.Load(); got != 1 {
		t.Fatalf("OnData calls after drain transition = %d, want 1", got)
	}

	reader.data = []byte("fresh")
	if err := conn.serve(events); err != nil {
		t.Fatalf("fourth serve() error = %v", err)
	}
	if got := calls.Load(); got != 2 {
		t.Fatalf("OnData calls after fresh data = %d, want 2", got)
	}
